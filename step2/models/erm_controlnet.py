"""
ERM-增强的ControlNet条件嵌入模块
=====================================
核心思想：将边缘增强模块(ERM)集成到 ControlNet 的 conditioning_embedding 中，
...
# ─────────────────────────────────────────────────────────────────────────────
# ERM 增强的 Conditioning Embedding
# ─────────────────────────────────────────────────────────────────────────────

class ControlNetConditioningEmbeddingWithERM(nn.Module):
    """
    ERM 增强的 ControlNet 条件嵌入模块
    在深层特征处插入 EdgeRefinementModule，增强分割边界的条件表达。
    """
    ...
    def __init__(
        ...
        self.erm = EdgeRefinementModule(deep_dim)
    ...
    def forward(self, conditioning: torch.Tensor) -> torch.Tensor:
        ...
        embedding = self.erm(embedding)

        embedding = self.conv_out(embedding)
        return embedding

    def load_backbone_from_standard(self, standard_embedding: nn.Module) -> None:
        """从标准 ControlNetConditioningEmbedding 加载主干权重（不包含 ERM）"""
        with torch.no_grad():
...
def apply_erm_to_controlnet(controlnet, input_resolution: int = 64):
    """
    就地给已有 ControlNet 替换为 ERM 增强版 conditioning_embedding。
    """
    ...
    erm_embedding = ControlNetConditioningEmbeddingWithERM(
        conditioning_embedding_channels=conditioning_embedding_channels,
        conditioning_channels=conditioning_channels,
        block_out_channels=block_out_channels,
    )

    erm_embedding.load_backbone_from_standard(orig_embed)

    controlnet.controlnet_cond_embedding = erm_embedding

    erm_params = sum(p.numel() for p in erm_embedding.erm.parameters())
    print(f"[OK] ERM 已注入 ControlNet conditioning_embedding  (ERM 新增参数: {erm_params:,})")
...
if __name__ == "__main__":
    ...
    print("\n" + "=" * 50)
    print("测试: ControlNetConditioningEmbeddingWithERM 前向传播")
    print("=" * 50)

    batch_size = 2
    H, W = 512, 512

    erm_embed = ControlNetConditioningEmbeddingWithERM(
    ...
    total = sum(p.numel() for p in erm_embed.parameters())
    erm_only = sum(p.numel() for p in erm_embed.erm.parameters())
    print(f"\n  总参数量: {total:,}")
    print(f"  ERM参数量: {erm_only:,}")
    print(f"  ERM占比: {erm_only/total*100:.1f}%")
