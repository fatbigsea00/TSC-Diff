"""
Mask-Guided Cross-Attention for ControlNet-Seg Training

Inspired by AeroGen's MaskCrossAttention.

Modulates UNet cross-attention output based on spatial region masks
extracted from the colormap conditioning image.
...
def set_mask_ca_data(
    ...
    _MASK_CA_STORE = store


def clear_mask_ca_data() -> None:
    global _MASK_CA_STORE
    _MASK_CA_STORE = {}
...
class MaskGuidedAttnProcessor:
    """Drop-in replacement for AttnProcessor2_0 on cross-attention layers.

    Reads modulation maps from the module-level _MASK_CA_STORE dict.
    ...
    def __call__(
        ...
        # ── Mask-guided output modulation ────────────────────────────
        if _MASK_CA_STORE:
            seq_len = hidden_states.shape[1]
            H = int(math.sqrt(seq_len))
            modulation = _MASK_CA_STORE.get(H)
...
def apply_mask_guided_attention(unet) -> None:
    """Replace all cross-attention (attn2) processors with MaskGuidedAttnProcessor."""
    attn_procs = {}
    count = 0
    for name, proc in unet.attn_processors.items():
        if "attn2" in name:
            attn_procs[name] = MaskGuidedAttnProcessor()
            count += 1
        else:
            attn_procs[name] = proc
    unet.set_attn_processor(attn_procs)
    print(f"  MaskGuidedCrossAttention: replaced {count} cross-attention processors")
...
def install_timestep_hook(unet) -> None:
    ...
    This hook reads the timestep arg and updates _MASK_CA_STORE['gate']
    ...
    def _hook(module, args, kwargs):
        ...
        gate_mid = getattr(module, "_mask_ca_gate_mid", 400.0)
        gate_temp = getattr(module, "_mask_ca_gate_temp", 100.0)

        t_float = t.float().mean()
        gate = torch.sigmoid((t_float - gate_mid) / gate_temp).item()

        # Update existing store's modulation maps with new gate
        if _MASK_CA_STORE and "base_obj" in _MASK_CA_STORE:
            for res in (64, 32, 16, 8):
                base_key = f"base_{res}"
                if base_key in _MASK_CA_STORE:
                    base = _MASK_CA_STORE[base_key]  # (obj_r, shd_r, ob, sb)
                    obj_r, shd_r, ob, sb = base
                    mod = 1.0 + gate * (ob * obj_r + sb * shd_r)
                    _MASK_CA_STORE[res] = mod.reshape(mod.shape[0], res * res, 1)

        _MASK_CA_STORE["gate"] = gate
...
def set_mask_ca_data_with_bases(
    colormap: torch.Tensor,
